    ...
    , m_comp_deriv(compute_deriv)
{
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
        m_axis >= 0 && m_axis < AMREX_SPACEDIM,
        "m_axis = " + std::to_string(m_axis) +
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
        dom_lo == 0, "dom_lo = " + std::to_string(dom_lo));
    int dom_hi2 = geom[0].Domain().bigEnd()[m_axis] + 1;
    for (int i = 0; i < finestLevel; ++i) {
        dom_hi2 *= mesh.refRatio(i)[m_axis];
    }
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
        dom_hi + 1 == dom_hi2, "dom_hi = " + std::to_string(dom_hi) +
                                   ", dom_hi2 = " + std::to_string(dom_hi2));
    ...
    amrex::Real x, int& ind, amrex::Real& c) const
{
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(
        ind >= 0 && ind + 1 < m_ncell_line,
        "ind = " + std::to_string(ind) +
            ", m_ncell_line = " + std::to_string(m_ncell_line));
}
